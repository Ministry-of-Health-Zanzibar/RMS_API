...
class DocumentTypeFormController extends Controller
{
    ...
    {
        $user = auth()->user();
        if (!$user->hasAnyRole(['ROLE ACCOUNTANT', 'ROLE ACCOUNTANT SUPPORT']) || !$user->can('View Document Form')) {
            return response([
                'message' => 'Forbidden',
                'statusCode' => 403
            ], 403);
    ...
    {
        $user = auth()->user();
        if (!$user->hasAnyRole(['ROLE ACCOUNTANT', 'ROLE ACCOUNTANT SUPPORT']) || !$user->can('Create Document Form')) {
            return response([
                'message' => 'Forbidden',
                'statusCode' => 403
            ], 403);
    ...
    {
        $user = auth()->user();
        if (!$user->hasAnyRole(['ROLE ACCOUNTANT', 'ROLE ACCOUNTANT SUPPORT']) || !$user->can('Update Document Form')) {
            return response([
                'message' => 'Forbidden',
                'statusCode' => 403
            ], 403);
    ...
    {
        $user = auth()->user();
        if (!$user->hasAnyRole(['ROLE ACCOUNTANT', 'ROLE ACCOUNTANT SUPPORT']) || !$user->can('Delete Document Form')) {
            return response([
                'message' => 'Forbidden',
                'statusCode' => 403
            ], 403);
