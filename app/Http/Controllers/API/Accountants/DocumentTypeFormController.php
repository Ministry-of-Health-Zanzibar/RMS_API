...
class DocumentTypeFormController extends Controller
{
    ...
    {
        $user = auth()->user();
        if (!$user->hasAnyRole(['ROLE ADMIN', 'ROLE NATIONAL', 'ROLE ACCOUNTANT']) || !$user->can('View Document Type')) {
            return response([
                'message' => 'Forbidden',
                'statusCode' => 403
            ], 403);
    ...
    {
        $user = auth()->user();
        if (!$user->hasAnyRole(['ROLE ADMIN', 'ROLE NATIONAL', 'ROLE ACCOUNTANT']) || !$user->can('Create Document Type')) {
            return response([
                'message' => 'Forbidden',
                'statusCode' => 403
            ], 403);
    ...
    {
        $user = auth()->user();
        if (!$user->hasAnyRole(['ROLE ADMIN', 'ROLE NATIONAL', 'ROLE ACCOUNTANT']) || !$user->can('Delete Document Type')) {
            return response([
                'message' => 'Forbidden',
                'statusCode' => 403
            ], 403);
