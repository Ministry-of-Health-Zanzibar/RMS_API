...
class SourceTypeController extends Controller
{
    ...
    {
        $user = auth()->user();
        if (!$user->hasAnyRole(['ROLE ACCOUNTANT', 'ROLE ACCOUNTANT SUPPORT']) || !$user->can('Create Source Type')) {
            return response([
                'message' => 'Forbidden',
                'statusCode' => 403
            ], 403);
    ...
    {
        $user = auth()->user();
        if (!$user->hasAnyRole(['ROLE ACCOUNTANT', 'ROLE ACCOUNTANT SUPPORT']) || !$user->can('Update Source Type')) {
            return response([
                'message' => 'Forbidden',
                'statusCode' => 403
            ], 403);
    ...
    {
        $user = auth()->user();
        if (!$user->hasAnyRole(['ROLE ACCOUNTANT', 'ROLE ACCOUNTANT SUPPORT']) || !$user->can('Delete Source Type')) {
            return response([
                'message' => 'Forbidden',
                'statusCode' => 403
            ], 403);
